...
class sAHP(Stimulus):
    ...
    def __init__(self,
        ...
        Args:
            ...
            long_amplitude (float): longer duration amplitude (nA)
            amplitude (float): middle amplitude (nA)
        ...
        self.long_amplitude = long_amplitude
    ...
    def instantiate(self, sim=None, icell=None, LFPyCell=None):
        ...
        times.append(self.delay)
        amps.append(self.holding_amplitude+self.long_amplitude)

        times.append(self.sahp_tmid)
        amps.append(self.holding_amplitude+self.long_amplitude)

        times.append(self.sahp_tmid)
        amps.append(self.holding_amplitude+self.amplitude)
    ...
    def __str__(self):
        ...
        return "sAHP pulse long_amplitude %f middle amplitude (amplitude) %f long_amplitude delay %f middle amplitude delay %f " \
            "totdur %f at %s" % (
                self.long_amplitude,
...
class PosCheops(Stimulus):
    ...
    def instantiate(self, sim=None, icell=None, LFPyCell=None):
        ...
        time_vec.append(self.delay+2*self.ramp1_dur+self.ramp12_delay+2*self.ramp2_dur+self.ramp23_delay+self.ramp3_dur)
        current_vec.append(self.holding_amplitude+self.ramp3_amp)

        time_vec.append(self.delay+2*self.ramp1_dur+self.ramp12_delay+2*self.ramp2_dur+self.ramp23_delay+2*self.ramp3_dur)
        current_vec.append(self.holding_amplitude)
    ...
    def __str__(self):
        ...
        return "PosCheops with ramp1_amp %f, ramp1_amp %f and ramp1_amp %f at %s" % (
            self.ramp1_amp,
            self.ramp2_amp,
            self.ramp3_amp,
            self.location)
