...
def define_recordings(protocol_name, protocol_definition, electrode=None):
    ...
    if "extra_recordings" in protocol_definition:
        ...
        for recording_definition in protocol_definition["extra_recordings"]:
            ...
            if recording_definition["type"] == "stimulus":
                location = ephys.locations.NrnSomaDistanceCompLocation(
                    name=recording_definition["name"],
                    soma_distance=recording_definition["somadistance"],
                    seclist_name=recording_definition["seclist_name"],
...
def define_stimuli(protocol_name, protocol_definition):
    ...
    for stimulus_definition in protocol_definition["stimuli"]:
        ...
        if protocol_name in ['sAHP']:
            stimuli.append(stimvalidation.sAHP(
                holding_amplitude=stimulus_definition['holding_amplitude'],
                long_amplitude=stimulus_definition['long_amplitude'],
                amplitude=stimulus_definition['amplitude'],
                location=soma_loc))
        
        if protocol_name in ['HyperDepol']:
            stimuli.append(stimvalidation.HyperDepol(
                holding_amplitude=stimulus_definition['holding_amplitude'],
                depol_amplitude=stimulus_definition['depol_amplitude'],
                hyperpol_amplitude=stimulus_definition['hyperpol_amplitude'],
                location=soma_loc))
        
        if protocol_name in ['PosCheops']:
            stimuli.append(stimvalidation.PosCheops(
                holding_amplitude=stimulus_definition['holding_amplitude'],
                ramp1_amp=stimulus_definition['ramp1_amp'],
                ramp2_amp=stimulus_definition['ramp2_amp'],
                ramp3_amp=stimulus_definition['ramp3_amp'],
                location=soma_loc))

        if protocol_name in ['NoiseOU3']:
            stimuli.append(stimvalidation.NoiseOU3(
                filename=stimulus_definition['filename'],
                location=soma_loc))
...
def define_all_protocols(feature_set=None, feature_file=None, electrode=None,
                     protocols_with_lfp=None):
    """
    Defines protocols for a specified feature_Set (or file)
